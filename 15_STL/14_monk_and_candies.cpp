...
int main()
{
    ...
    multiset<long long> mst; //see the given contraint for data type
    ...
    {
        int n, k;
        cout << "enter total bags and total minutes : ";
        cin >> n >> k;

        cout << "enter candies in " << n << " bags : ";
        for (int i = 0; i < n; i++)
        {
            cin >> temp;
            mst.insert(temp);
        ...
        for (int i = 0; i < k; i++)
        {
            auto it = mst.end();
            it--;
            long long curr_max = *it;

            total_candies += curr_max;

            mst.erase(it); //dont erase the curr_max this will erase everything
            //which is equal to curr_max

            mst.insert(curr_max / 2);
        }

        cout << "maximum candies eaten : " << total_candies << endl;
    }
    return 0;
}
